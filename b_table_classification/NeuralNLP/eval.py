...
def eval(conf):
    ...
    # added by zzh
    # 
    # please replace all `np.int' and `np.float' with `int' and `float'
    if conf.task_info.hierarchical:
        label_01_standard = 1*np.array([[i in standard_label for i in range(2, len(empty_dataset.label_map))] for standard_label in standard_labels])
        label_01_predict = 1*(np.array(predict_probs)[:, 2:]>conf.eval.threshold)
    else:
        label_01_standard = 1*np.array([[i in standard_label for i in range(len(empty_dataset.label_map))] for standard_label in standard_labels])
        label_01_predict = 1*(np.array(predict_probs)>conf.eval.threshold)
    conf_mat, normal_conf_mat = mlcm.cm(label_01_standard, label_01_predict)
    one_vs_rest = mlcm.stats(conf_mat)
    # pickle.dump([empty_dataset.label_map, conf_mat, normal_conf_mat, one_vs_rest],
    #             open(evaluator.eval_dir + '/' + 'mlcm.pkl', 'wb'))
