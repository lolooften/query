...
class Predictor(object):
    ...
    def predict(self, texts):
        ...
        with torch.no_grad():
            ...
            if self.config.model_name == 'TextRNN' and self.config.TextRNN.doc_embedding_type == 'Attention':
                logits, alpha = self.model(batch_texts)
                alpha = alpha.cpu().tolist()
                with open(self.config.checkpoint_dir + "/attention_weight.txt", "w") as of:
                    for line in alpha:
                        of.write(','.join(str(w) for w in line) + '\n')
            else:
...
def predict(config_file, tokens_list):
    ...
    for tokens in tokens_list:
        # property names 必须是双引号, 因此不能用 str, 需要用 json.dumps
        input_texts.append(json.dumps({
            'doc_label': [],
            'doc_token': tokens,
            'doc_keyword': [],
            'doc_topic': [],
    ...
    for predict_prob in predict_probs:
        ...
        predict_label_names.append(predict_label_name)
    return predict_label_names
    
if __name__ == "__main__":
    # config = Config(config_file=sys.argv[1])
    config_list = [Config(config_file='conf/train_table.json'), Config(config_file='conf/train_time.json')]
    for config in config_list:
        ...
        predict_probs = []
        is_multi = config.task_info.label_type == ClassificationType.MULTI_LABEL
        for line in codecs.open(config.data.test_json_files[0], "r", predictor.dataset.CHARSET):
        ...
        for i in range(epoches):
            ...
            for j in predict_prob:
                predict_probs.append(j)
        with codecs.open(config.checkpoint_dir + "/predict.txt", "w", predictor.dataset.CHARSET) as of:
            for predict_prob in predict_probs:
                if not is_multi:
                    predict_label_ids = [predict_prob.argmax()]
                else:
