...
class Editor(ScrolledText):
    def __init__(self, parent, entity_pattern, recommend_pattern):
        ...
        fnt = font.Font(family='Inconsolata', size=14, underline=0)
...
class KeyMapFrame(Frame):
    ...
    def create_widgets(self):
        title = Label(self, text="Shortcuts map", foreground="blue", font=(self.textFontStyle, 12, "bold"))
        title.grid(row=0, column=0, columnspan=2, sticky=W, padx=6, pady=8)
        for row, item in enumerate(self.keymap, 1):
            key_lbl = Label(self, text=item.key.upper() + ": ", font=(self.textFontStyle, 12, "bold"))
            key_lbl.grid(row=row, column=0, sticky=NW, padx=4, pady=4)
            self.key_labels.append(key_lbl)

            name_entry = Entry(self, font=(self.textFontStyle, 12), bg=item.color)
...
class Application(Frame):
    ...
    def initUI(self):
        ...
        self.configListBox.grid(row=8, column=self.textColumn + 1, columnspan=1)
